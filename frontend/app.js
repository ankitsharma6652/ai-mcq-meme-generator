...
function App() {
    ...
    const [memeInputType, setMemeInputType] = useState('topic'); // 'topic' or 'url'
    const [memeUrlInput, setMemeUrlInput] = useState('');
    const [memeType, setMemeType] = useState('image'); // 'image', 'gif', or 'video'
    ...
    const handleGenerateMeme = async () => {
        // Validate based on input type
        if (memeInputType === 'topic' && !memeTopic) {
            setError("Please enter a topic for the meme.");
            return;
        }
        if (memeInputType === 'url' && !memeUrlInput) {
        ...
        <div className="container">
            ...
            <div className="card">
                ...
                {inputType !== 'meme' ? (
                    ...
                    >
                        {memeLoading ? <span className="material-icons" style={{ marginRight: '0.5rem' }}>cancel</span> : <span className="material-icons" style={{ marginRight: '0.5rem' }}>brush</span>}
                        {memeLoading ? `Cancel Generation (${timer.toFixed(1)}s)` : 'Dreaming up a meme...'}
                    </button>
                )}
            </div>

            {/* Meme Result */}
            {memeImages.length > 0 && inputType === 'meme' && (
                <div className="card" style={{ textAlign: 'center' }}>
                    <h2 style={{ marginBottom: '1.5rem' }}>Your AI Generated Memes</h2>
                    <div style={{
                        display: 'grid',
...
function MCQItem({ mcq, index, shareQuestion, shareToLinkedIn, shareToTwitter, shareToFacebook, shareToWhatsApp, shareToTelegram, shareToReddit, shareViaEmail }) {
        ...
        <div className="mcq-item">
            <div className="mcq-header">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', position: 'relative' }}>
                    {mcq.tags.map(tag => <span key={tag} className="tag">{tag}</span>)}
